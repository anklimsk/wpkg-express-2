 ...
 */
	public function requestJob($capabilities, $group = null) {
		...
		foreach ($data as $item) {
			$whereClause[] = '(id = ' . $item[$this->alias]['id'] . ' AND (workerkey IS NULL OR fetched <= ' . $timestampConv . '\'' . date('Y-m-d H:i:s', time() - $capTimeout[$item[$this->alias]['jobtype']]) . '\'))';
		...
		$query = 'UPDATE ' . $this->tablePrefix . $this->table . ' SET workerkey = "' . $key . '", fetched = "' . date('Y-m-d H:i:s') . '" WHERE ' . implode(' OR ', $whereClause) . ' ORDER BY ' . $this->virtualFields['age'] . ' ASC, id ASC LIMIT 1';
		if ($this->isDataSourcePostgres()) {
			$query = 'UPDATE ' . $this->tablePrefix . $this->table . ' SET workerkey = \'' . $key . '\', fetched = \'' . date('Y-m-d H:i:s') . '\' FROM ' .
			'(SELECT ' . $this->primaryKey . ' FROM ' . $this->tablePrefix . $this->table .
			' WHERE ' . implode(' OR ', $whereClause) . ' ORDER BY ' . $this->virtualFields['age'] . ' ASC, id ASC LIMIT 1 FOR UPDATE) t ' .
			'WHERE t.' . $this->primaryKey . ' = ' . $this->tablePrefix . $this->table . '.' . $this->primaryKey;
		}
 ...
 */
	public function getTypes() {
		...
		return array_values($this->find('list', $findCond));
	}
 ...
 */
	public function getStats() {
		...
		if ($this->isDataSourcePostgres()) {
			$findCond['fields'] = [
			'jobtype,count(id) as num, AVG(EXTRACT(EPOCH FROM (completed::timestamp - created::timestamp))) AS alltime, AVG(EXTRACT(EPOCH FROM (completed::timestamp - fetched::timestamp))) AS runtime,' .
				'CASE
					WHEN notbefore IS NULL THEN AVG(EXTRACT(EPOCH FROM (fetched::timestamp - created::timestamp)))
					ELSE AVG(EXTRACT(EPOCH FROM (fetched::timestamp - notbefore::timestamp)))
				END AS fetchdelay'
			];
		}
 ...
 * Truncate table.
 *
 * @param string|null $table A string representing the table to be truncated
