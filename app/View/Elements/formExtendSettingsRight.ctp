...
if (!empty($varsExt)) {
	...
	echo $this->Form->inputs([
		...
		'Setting.SmbLogShare' => ['label' => __('Share name containing logs') . ':', 'title' => __('The name of the SMB share containing the log files, e.g.: wpkg/log.'),
			'type' => 'text', 'data-toggle' => 'tooltip'],
		'Setting.SmbDbShare' => ['label' => __('Share name containing client databases') . ':', 'title' => __('The name of the SMB share containing the client database files, e.g.: wpkg/db.'),
			'type' => 'text', 'data-toggle' => 'tooltip'],
	]);