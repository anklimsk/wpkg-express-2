...
if (!isset($isAddAction)) {
	...
	$inputList = [
		...
		$modelName . '.reboot_id' => ['label' => [__('Reboot'), __('If set, the system will reboot after installation of this package.'), ':'],
			'type' => 'select', 'data-toggle' => 'tooltip', 'options' => $listReboot, 'autocomplete' => 'off'],
		$modelName . '.execute_id' => ['label' => [__('Execute'), __('&quot;Always&quot; will always execute all of this package\'s Install actions every time, whereas &quot;Once&quot; will only execute them once. Both of these modes will always ignore all Package checks.'), ':'],
			'type' => 'select', 'data-toggle' => 'tooltip', 'options' => $listExecute, 'autocomplete' => 'off'],
		$modelName . '.notify_id' => ['label' => [__('Notify'), __('If set, the system will notify the user via &quot;net send&quot; when installation starts and stops.'), ':'],
