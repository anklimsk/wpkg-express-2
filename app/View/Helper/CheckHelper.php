			...
			case CHECK_TYPE_FILE:
				...
				switch ($data['condition']) {
					...
					case CHECK_CONDITION_FILE_SIZE_EQUALS:
						$size = $this->Number->toReadableSize((float)$data['value']);
						$sizeText = $this->Html->tag(
							'span',
							$size,
							[
								'data-toggle' => 'tooltip',
								'title' => $this->Number->format($data['value']) . ' ' . __('bytes'),
								'class' => 'help'
							]
						);
			...
			case CHECK_TYPE_HOST:
				switch ($data['condition']) {
					...
					case CHECK_CONDITION_HOST_OS:
						$constVal = constValToLcSingle('ATTRIBUTE_OS_', $data['value'], ' ', false, false);
						if (empty($constVal)) {
							$constVal = h($data['value']);
						}
						$conditionName = __("Host OS is '%s'", $constVal);
						break;
					case CHECK_CONDITION_HOST_ARCHITECTURE:
						$constVal = constValToLcSingle('ATTRIBUTE_ARCHITECTURE_', $data['value'], ' ', false, false);
						if (empty($constVal)) {
							$constVal = h($data['value']);
						}
						$conditionName = __("Host architecture is '%s'", $constVal);
						break;
					...
					case CHECK_CONDITION_HOST_LCID_OS:
						if (!empty($data['value'])) {
							...
							unset($itemLcId);
							$value = $this->Text->toList($listLcId, __('and'));
						}
